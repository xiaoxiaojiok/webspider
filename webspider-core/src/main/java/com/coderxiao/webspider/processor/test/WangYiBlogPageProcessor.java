...
public class WangYiBlogPageProcessor implements PageProcessor {

	private Site site = Site.me().setRetryTimes(3).setSleepTime(1)
			.setCycleRetryTimes(3).setDepth(2);
	...
	public void process(Page page) {
		...
		List<String> list = page.getHtml()
				.links()
				.regex("http://[a-zA-Z0-9]+.blog.163.com/blog/static/[0-9]+/")
				.all();
		for (String url : list) {
	...
	public static void main(String[] args) {
		...
		Spider spider = Spider.create(new WangYiBlogPageProcessor());
		spider.addPipeline(new HTMLPipeline("D:\\webmagic\\"))
		
				.addUrl("http://blog.163.com/")
				.scheduler(scheduler)
