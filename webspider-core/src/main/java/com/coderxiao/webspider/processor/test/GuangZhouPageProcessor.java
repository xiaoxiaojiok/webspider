...
public class GuangZhouPageProcessor implements PageProcessor {
	...
	public void process(Page page) {
		...
		List<String> list = page.getHtml()
				.links()
				.regex("http://www.020.com/thread-\\d+-\\d+-\\d+.html")
				.all();
		for (String url : list) {
	...
	public static void main(String[] args) {
		...
		Spider spider = Spider.create(new GuangZhouPageProcessor());
		spider.addPipeline(new HTMLPipeline("D:\\webmagic\\"))
				.addUrl("http://www.020.com/")
				.scheduler(scheduler)
